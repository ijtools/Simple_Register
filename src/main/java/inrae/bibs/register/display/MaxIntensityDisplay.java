...
/**
 * Displays the combination of two input images by computing the maximum ofthe
 * intensity of each imgae.
 * 
 * @author dlegland
 *
 */
public class MaxIntensityDisplay implements ImagePairDisplay
{
    @Override
    public ImageProcessor compute(ImageProcessor image1, ImageProcessor image2)
    {
        ...
        for (int y = 0; y < sizeY; y++)
        {
            for (int x = 0; x < sizeX; x++)
            {
                ...
                result.setf(x, y, Math.max(v1, v2));
            }
        }
