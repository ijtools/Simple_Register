...
import inrae.bibs.register.interp.Function2D;
import inrae.bibs.register.interp.Function3D;
import inrae.bibs.register.interp.TransformedImage2D;
import inrae.bibs.register.interp.TransformedImage3D;
...
public class Registration
{
     ...
     */
    public static final ImageProcessor computeTransformedImage(ImageProcessor reference, Transform2D transfo, ImageProcessor movingImage)
    {
        ...
        Function2D interp = new TransformedImage2D(movingImage, transfo);
        ...
        for (int y = 0; y < sizeY; y++)
        {
            ...
            {
                result.setf(x, y, (float) interp.evaluate(x, y));
            }
        }
        return result;
    }
     ...
     * @return a new image with the same size as the reference image
     */
    public static final Image3D computeTransformedImage(Image3D reference, Transform3D transfo, Image3D movingImage)
    {
        // retrieve result image size
...
//                IJ.log("y = " + y);
                ...
                {
//                    IJ.log("x = " + x);
                    Point3D p = transfo.transform(new Point3D(x, y, z));
...
//                    IJ.log("update value");
     ...
     * @return a new image with the same size as the reference image
     */
    public static final ImageStack computeTransformedImage(ImageStack refStack, Transform3D transfo, ImageStack movingStack)
    {
        // retrieve result image size
        ...
        // Create interpolation class, that encapsulates both the image and the transform
        Function3D interp = new TransformedImage3D(movingStack, transfo);
        ...
        for (int z = 0; z < sizeZ; z++)
        {
            ...
            for (int y = 0; y < sizeY; y++)
            {
                for (int x = 0; x < sizeX; x++)
                {
                    result.setValue(x, y, z, interp.evaluate(x, y, z));
                }
            }
        }
        IJ.showStatus("image transformed");
